...
import os
import cPickle
from network_architecture_2D import build_net
...
def run(fold=0):
    ...
    net, seg_layer = build_net(x_sym, (128,128), num_classes, 4, 16, batch_size=BATCH_SIZE*128,
...
#            data = data_dict["data"].astype(np.float32)
            data = data_dict["data"].astype(np.float32).transpose(0, 2, 3, 4, 1).reshape((-1, 128, 128, 4))
            data = data.transpose(0, 3, 1, 2)

            seg_classes = data_dict["seg_onehot"].astype(np.float32).shape[1]
            print("seg_classes", seg_classes)
            seg = data_dict["seg_onehot"].astype(np.float32).transpose(0, 2, 3, 4, 1).reshape((-1, 128, 128, seg_classes))
            seg = seg.reshape((-1, num_classes))
